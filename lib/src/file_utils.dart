...
/// Set to true to enable detailed debug logs, false to disable.
...
/// [acceptedFileTypes] List of accepted file types (e.g., '.pdf', 'image/*').
///
/// Returns a Future that resolves to a List of native web File objects.
/// Returns an empty list if selection is canceled.
Future<List<web.File>> selectWebFiles({
  ...
  // This is a safeguard in case the change event doesn't fire
  // Most browsers will only fire change when files are selected, not on cancel
...
///
/// This class provides an XFile interface for native web File objects,
/// allowing them to be used with TusClient and other libraries expecting XFile.
/// It implements streaming that never loads the entire file into memory.
class NativeWebXFile extends XFile {
  final web.File _file;
  final String? _mimeType;

  /// Creates a new NativeWebXFile wrapping a native web.File.
  ///
  /// [file] The native web File object to wrap.
  /// [mimeType] Optional MIME type override. If not provided, the File's type is used.
  NativeWebXFile(web.File file, {String? mimeType})
      : _file = file,
        _mimeType = mimeType ?? file.type,
        super(
          '', // Empty path since we use the native File object directly
          name: file.name,
          length: file.size,
          mimeType: mimeType ?? file.type,
          bytes: null, // Explicitly null since we don't want to load into memory
          lastModified: file.lastModified != 0 
              ? DateTime.fromMillisecondsSinceEpoch(file.lastModified) 
              : null, // Use the file's lastModified timestamp if available
        ) {
    if (kTesting) {
  ...
  // Creates a stream by slicing the File directly for the requested range.
  Stream<Uint8List> _createBlobSliceStream(int start, int end) async* {
    if (kTesting)
      print(
          '[NativeWebXFile] _createBlobSliceStream called with start: $start, end: $end');
    ...
    if (kTesting)
      print(
          '[NativeWebXFile] Yielding sliced bytes of length: ${bytes.length}');
    yield bytes;
  }
  ...
  Future<int> length() async {
    if (kTesting)
      print(
          '[NativeWebXFile] length() called. Returning file size: ${_file.size}');
    return _file.size;
  }
}
...
///
/// This function creates a streaming-optimized XFile wrapper around a native
/// web.File object, allowing efficient handling of large files.
///
/// [file] The native web File object to convert.
...
///
/// This is a convenience function that combines selecting files and converting
/// them to XFiles in a single operation.
///
/// [allowMultiple] Whether to allow selecting multiple files.
/// [acceptedFileTypes] List of accepted file types (e.g., '.pdf', 'image/*').
///
/// Returns a List of XFile objects that can be used with TusClient.
